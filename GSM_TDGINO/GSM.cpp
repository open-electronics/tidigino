...
{
  // open the Serial1 line for the communication
			...
			DebugPrint("DEBUG: the baud is not ok\r\n", 0);
			  ...
			  for (int i=1;i<7;i++){
				  ...
				  /*sprintf (buff,"AT+IPR=%f",baud_rate);
					...
					#endif
					*/
				  Serial1.print("AT+IPR=");
				  Serial1.print(baud_rate);    
				  Serial1.print("\r"); // send <CR>
				  delay(500);
				  Serial1.begin(baud_rate);
				  delay(100);
...
void GSM::InitParam(byte group)
{
  ...
  switch (group) {
    case PARAM_SET_0:
      ...
      SendATCmdWaitResp("AT+IPR=115200", 500, 50, "OK", 5);
      // setup mode
...
void GSM::Call(int sim_position)
{
  ...
  SetCommLineStatus(CLS_ATCMD);
  // ATD>"SM" 1;<CR>
  Serial1.print("ATD>\"SM\" ");
  Serial1.print(sim_position);    
  Serial1.print(";\r");
...
char GSM::SetSpeakerVolume(byte speaker_volume)
{
  ...
  // select speaker volume (0 to 14)
  // AT+CLVL=X<CR>   X<0..14>
  Serial1.print("AT+CLVL=");
  Serial1.print((int)speaker_volume);    
  Serial1.print("\r"); // send <CR>
  // 10 sec. for initial comm tmout
  // 50 msec. for inter character timeout
...
char GSM::IsSMSPresent(byte required_status) 
{
  ...
  switch (required_status) {
    case SMS_UNREAD:
      Serial1.print("AT+CMGL=\"REC UNREAD\"\r");
      break;
    case SMS_READ:
      Serial1.print("AT+CMGL=\"REC READ\"\r");
      break;
    case SMS_ALL:
